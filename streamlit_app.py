...
def recommendation_engine():
    merged_df_tr = pd.read_pickle(get_path("Streamlit Data", "Recommendations", "merged_df_tr.bz2"))
    rating_crosstab = pd.read_pickle(get_path("Streamlit Data", "Recommendations", "rating_crosstab.bz2"))
    coss_mat = np.load(get_path("Streamlit Data", "Recommendations", "coss_mat.npz"))
    coss_mat = coss_mat["a"]
...
def association_analyzer():
    rules = pd.read_pickle(get_path("Streamlit Data", "Market Basket Analysis", "rules.bz2"))
    return rules


@st.cache_resource
def churn_analyzer():
    gbc = joblib.load(get_path("Streamlit Data", "Churn", "gbc.joblib"))
    dt = joblib.load(get_path("Streamlit Data", "Churn", "dt.joblib"))
    lr = joblib.load(get_path("Streamlit Data", "Churn", "lr.joblib"))
    rf = joblib.load(get_path("Streamlit Data", "Churn", "rf.joblib"))
    stack = joblib.load(get_path("Streamlit Data", "Churn", "stack.joblib"))
    X = np.load(get_path("Streamlit Data", "Churn", "X.npy"))
    y = np.load(get_path("Streamlit Data", "Churn", "y.npy"))


    with open(get_path("Streamlit Data", "Churn", "scaler"), "rb") as f:
        scaler = pickle.load(f)
    churn_df_sample = pd.read_pickle(get_path("Streamlit Data", "Churn", "churn_df_sample.bz2"))
...
@st.cache_resource
def clustering():
    cluster_data_2 = pd.read_pickle(get_path("Streamlit Data", "Customer Segmentation", "Clustering", "cluster_data_2.bz2"))
    cluster_data_3 = pd.read_pickle(get_path("Streamlit Data", "Customer Segmentation", "Clustering", "cluster_data_3.bz2"))
    cluster_data_4 = pd.read_pickle(get_path("Streamlit Data", "Customer Segmentation", "Clustering", "cluster_data_4.bz2"))
    cluster_data_5 = pd.read_pickle(get_path("Streamlit Data", "Customer Segmentation", "Clustering", "cluster_data_5.bz2"))
    with open(get_path("Streamlit Data", "Customer Segmentation", "Clustering", "X_new"), "rb") as X_new_file:
...
@st.cache_resource
def rfm_loader():
    rfm = pd.read_pickle(get_path("Streamlit Data", "Customer Segmentation", "RFM", "rfm.bz2"))
    return rfm
...
st.set_page_config(
    page_title="Retail Analytics and Recommendation System - Powered by Intelligent Machines",
    page_icon="🛍️",
    layout="wide",
...
with main2:
    st.image(Image.open(get_path("Streamlit Data", "Images", "color_logo.png")), width=150)
    st.title("Retail Analytics and Recommendation System")
    st.markdown("""
                Hi, I'm Arjun Luthra, a data enthusiast from India! The code for this web app can be found [here](https://github.com/Arjun-Luthra-alt/Retail-Analytics-and-Recommendation-System).\n
    ...
    if question == 'What products do we recommend to our users?':
        ...
        st.image(Image.open(get_path("Streamlit Data", "Images", "book.png")), width=50)
        ...
        with st.expander("Learn more about Cosine Similarity"):
            st.markdown('''
                        ...
                        ''')
            st.image(Image.open(get_path("Streamlit Data", "Images", "cosine-similarity.png")), width=500)
    ...
    if question == 'What products increase the chance of buying other products?':
        ...
        st.write('''
                ...
                ''')
        st.image(Image.open(get_path("Streamlit Data", "Images", "grocery.png")), width=50)
        ...
        st.markdown('''
                    ...
                    ''')
        st.image(get_path("Streamlit Data", "Images", "equations-mba.png"),
    ...
    if question == 'What customers are we likely to lose soon?':
        ...
        if option == 'Gradient Boosting Classifier':
            ...
            st.image(Image.open(get_path("Streamlit Data", "Images", "boosting.png")),
        ...
        if option == 'Decision Tree':
            ...
            st.image(Image.open(get_path("Streamlit Data", "Images", "rain.png")),
